...
include { FASTQ_CONVERT          } from '../modules/local/fastq_convert'
...
include { methodsDescriptionText } from '../subworkflows/local/utils_nfcore_bulk_m6A_pipeline'
...
workflow BULK_M6A {
    ...
    def genome_name = null
    def genome_dir = "${workDir}/temp_dir/genome"
    ...
    // fastq convert
    if (params.run_fastqConvert){
        FASTQ_CONVERT(ch_samplesheet)
        ch_samplesheet = FASTQ_CONVERT.out.convert_R1.mix(FASTQ_CONVERT.out.convert_R2).groupTuple()
    }
    ...
    // quant
    ch_merge = CONVERSION_SUMMARY.out.conv_sample.join(CONVERSION.out.conv_wellbam.groupTuple())
    ch_merge = ch_merge.join(CONVERSION_SUMMARY.out.conv_loci).join(STARSOLO.out.raw_matrix)
    QUANT(
        ch_merge
    )
    ...
    ch_merge = ch_merge.join(CONVERSION_SUMMARY.out.conv_motif)
