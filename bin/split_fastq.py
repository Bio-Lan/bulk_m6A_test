...
def splitInf_to_dict(file,sample):
    ...
    if '_'.join(df.columns) != "raw_sample_well_sub_sample":
        sys.exit(f"Wrong file,header should be(TAB-delimiter):\n raw_sample\twell\tsub_sample")
...
class Split_Fastq:
    ...
    def run(self):
        ...
        out_json = raw_sample + ".bulk_m6A.well_bc.json"
        utils.write_json(sub_bc,out_json)
        out_json = raw_sample + ".bulk_m6A.fastq_inf.json"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--protocol", required=True,default='bulk_m6A-V1')
