...
```
pip install sccore
manifest -m manifest.csv -f /workspaces/bulk_m6A_test_data/bulk_m6A-V1
```

Recursively search the specified folders for fastq files and (optional) matched barcode files.

`-m --manifest` Path to the manifest CSV file containing mappings between fastq file prefixes and sample names. An example `manifest.csv` can be found in the [test data repository](https://github.com/singleron-RD/bulk_m6A_test_data/tree/main/bulk_m6A-V1).
...
```bash
nextflow run singleron-RD/bulk_m6A \
...
```bash
nextflow run singleron-RD/bulk_m6A -profile docker -params-file params.yaml
```
...
```
pip install nf-core
nf-core launch singleron-RD/bulk_m6A
```
...
```
`genome_name` can also be a path to store the STAR index files at a specified location
...
```
nextflow run singleron-RD/bulk_m6A -profile test,docker --outdir results
```
...
```bash
nextflow pull singleron-RD/bulk_m6A
```
...
> ```
> git clone https://github.com/singleron-RD/bulk_m6A.git
> nextflow run /workspace/pipeline/bulk_m6A ...
> ```
...
First, go to the [singleron-RD/bulk_m6A releases page](https://github.com/singleron-RD/bulk_m6A/releases) and find the latest pipeline version - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`. Of course, you can switch to another version by changing the number after the `-r` flag.
