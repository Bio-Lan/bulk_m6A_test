...
| `m6A_file` | m6A position file.<details><summary>Help</summary><small>If want to valid for all wells, must haver column name 'chrom' and 'loci', comma separated.<br>If want to for each well, one well per line, no  column name, the format is "well,path_to_m6A_file", path_to_m6A_file must haver column name 'chrom' and 'loci', comma separated.</small></details> | `string` | | | |
